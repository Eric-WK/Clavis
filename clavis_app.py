...
keywords_file = keywords_col.file_uploader("Upload your keywords file", type=["xlsx", "xls", "csv"])
    ...
    if keywords_file is None:
        ...
        with st.spinner("Expanding Keywords..."):
            ...
            try: 
                ...
                parsed_search_volume, expansion_factor = parse_search_volume(search_volume, PAYLOAD)
                ...
                st.error(e)
                # break
        st.metric("Expansion Factor", value=round(st.session_state['expansion_factor'],2), delta = 0, delta_color = "normal")
...
if clavis_button:
    if config_file is None: 
        ...
        categorized_keywords = categorize_keywords(st.session_state.parsed_search_volume, categorization_df)
        
        ## add the categorized keywords to the session state
        st.session_state.categorised_keywords = categorized_keywords
        
        ## now URL generation 
        generated_urls = categorize_keywords(st.session_state.parsed_search_volume, url_generation_df)

        ## get the columns that are not keywords, search volume, idea, or 0
        columns_to_concatenate = generated_urls.columns[~generated_urls.columns.isin(["keywords", "search_volume", "idea"])]

        ## in all of the columns to concatenate, replace 0 with "", 0 is a str ("0")
        generated_urls[columns_to_concatenate] = generated_urls[columns_to_concatenate].replace("0", "")

        # ## concatenate the columns
        generated_urls["url"] = generated_urls[columns_to_concatenate].apply(lambda x: "-".join(x.dropna().astype(str)), axis=1)

        ## cleaning - remove the hyphen 
        generated_urls['url'] = generated_urls['url'].replace("-","", regex=True, inplace=False)

        ## replace the ampersand & with ""
        generated_urls['url'] = generated_urls['url'].replace("&","", regex=True, inplace=False)

        ## join back the two dataframes
        final_df = pd.merge(categorized_keywords, generated_urls, on=["keywords","search_volume","idea"], how="left")
