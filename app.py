...
def new_column_from_map(
    dataframe: pd.DataFrame, dictionary: Dict[str, str], anchor_col: str
) -> pd.DataFrame:
...
def display_dict_df(dictionary: Dict[str, str]) -> pd.DataFrame:
    ...
    return (
        ...
        .T.reset_index()
        .rename(columns={"index": "SearchFor", 0: "Return"})
    )


def check_upload_state(
    keywords_upload: st.button, categories_upload: st.button
) -> bool:
...
def load_keywords(file_name: str) -> pd.DataFrame:
    ...
    return pd.read_csv(file_name, sep="\t", header=None, dtype=str).rename(
        columns={0: "keywords"}
    )
...
def load_categories(file_name: str) -> pd.DataFrame:
    ...
    return pd.read_csv(file_name, sep=";", dtype=str).rename(
        columns={"Search For": "SearchFor"}
    )
...
def map_categories(dataframe: pd.DataFrame) -> Dict[str, str]:
    ...
    for category in dataframe["Category"].unique():
        ...
        ## create a dictionary to map the categories
        mapped_categories[category] = dict(
            zip(filtered["SearchFor"], filtered["Return"])
        )
...
def run_clavis() -> pd.DataFrame:
    ...
    keyword_dataframe = new_column_from_map(
        keyword_dataframe, mapped_categories, "keywords"
    )
